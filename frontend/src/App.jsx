...
export default function App() {
  ...
  const handleAddEdge = () =>
    setEdges([...edges, { from: "", to: "", value: 0 }]);
  ...
  const handleOptimize = async () => {
    try {
      ...
      Object.entries(res.data).forEach(([key, flow]) => {
        ...
        if (flow > 0) {
          explain.push(
            `По каналу ${from} → ${to} оптимально передано ${flow} единиц потока`
          );
        }
      });
      ...
      alert("Ошибка оптимизации. Проверь структуру сети.");
      console.error(e);
    }
  };
  ...
  return (
    <div className="app-container">
      ...
      <div className="template-section">
        <h2>Шаблоны сетей</h2>
        <button onClick={() => loadTemplate("linear")}>Линейная</button>
        <button onClick={() => loadTemplate("star")}>Звезда</button>
        <button onClick={() => loadTemplate("full")}>Полносвязная</button>
        <button onClick={() => loadTemplate("tree")}>Иерархическая</button>
        <button onClick={() => loadTemplate("transport")}>Транспортная</button>
      </div>
